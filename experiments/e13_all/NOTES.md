...
--label-smoothing 0.05`, no WikiDocs cap, plus `--enriched --feature-blocks "prom,name,cue,sib,geo,shape,cf"`
(30 extra columns on gaz_info).
...
for S in 42 101 202 617 1848; do
  WANDB_MODE=offline uv run python tools/train.py train \
    ...
    --seed $S --enriched --feature-blocks "prom,name,cue,sib,geo,shape,cf" \
    --metrics-out experiments/e13_all/seed$S.json > experiments/e13_all/seed$S.log 2>&1
done
```
...
All seven blocks. +0.0334 EM / +0.0256 acc@161 over e11b, and every one of the six sources improves significantly -- including Prodigy (+0.043), the source that had been fighting the recipe through all of wave 1. This single change is larger than everything wave 1 achieved put together.
