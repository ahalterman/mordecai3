...
--label-smoothing 0.05`, no WikiDocs cap, plus `--enriched --feature-blocks "prom,name,cue,sib,geo,shape"`
(26 extra columns on gaz_info).
...
for S in 42 101 202 617 1848; do
  WANDB_MODE=offline uv run python tools/train.py train \
    ...
    --metrics-out experiments/e14_no_cf/seed$S.json > experiments/e14_no_cf/seed$S.log 2>&1
done
```
...
**Recommended.** All blocks except cf. Best arm in the wave: +0.0345 EM / +0.0269 acc@161 over e11b. Beats e13_all on acc@161 by a small but significant margin (+0.0013 ± 0.0013) and ties it on EM, with four fewer features. Prodigy gains most of any arm (+0.0545).
