...
no WikiDocs cap. This arm: SWA at 15 epochs, average epochs 8-15 (`--epochs 15 --avg-params --avg-mode swa`).
...
for S in 42 101 202 617 1848; do
  WANDB_MODE=offline uv run python tools/train.py train --mix-dim 512 \
    ...
    --checkpoint-out experiments/e29_swa_ep15/seed$S.pt \
    --metrics-out experiments/e29_swa_ep15/seed$S.json > experiments/e29_swa_ep15/seed$S.log 2>&1
done
```
...
**Best training configuration of the campaign.** SWA at the original 15 epochs, averaging epochs 8-15 -- more points in the average than e25_swa gets. +0.0067 EM* / +0.0029 acc@161* over e14_no_cf, and the only arm in the whole campaign that improves *every one of the six sources* (Prodigy +0.0031*, TR +0.0052, LGL +0.0131*, GWN +0.0097*, Synth +0.0040*, WikiDocs +0.0052*).
