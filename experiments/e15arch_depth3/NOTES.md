...
no WikiDocs cap; this arm adds `--mix-depth 3`.
...
for S in 42 101 202 617 1848; do
  WANDB_MODE=offline uv run python tools/train.py train --epochs 15 --mix-dim 512 \
    ...
    --mix-depth 3 --seed $S \
    --metrics-out experiments/e15arch_depth3/seed$S.json > experiments/e15arch_depth3/seed$S.log 2>&1
done
```
...
Third mix layer: significantly worse on acc@161 (-0.0026*) and worse on TR (-0.0093*).
