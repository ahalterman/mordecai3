...
no WikiDocs cap; this arm adds `--mix-depth 3 --residual`.
...
for S in 42 101 202 617 1848; do
  WANDB_MODE=offline uv run python tools/train.py train --epochs 15 --mix-dim 512 \
    ...
    --mix-depth 3 --residual --seed $S \
    --metrics-out experiments/e15arch_depth3res/seed$S.json > experiments/e15arch_depth3res/seed$S.log 2>&1
done
```
...
Third layer with residual connections: recovers most of what depth3 lost, still no gain.
