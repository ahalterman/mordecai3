...
--label-smoothing 0.05`, no WikiDocs cap, plus `--enriched --feature-blocks "cf"`
(4 extra columns on gaz_info).
...
for S in 42 101 202 617 1848; do
  WANDB_MODE=offline uv run python tools/train.py train \
    ...
    --seed $S --enriched --feature-blocks "cf" \
    --metrics-out experiments/e13_cf/seed$S.json > experiments/e13_cf/seed$S.log 2>&1
done
```
...
Case-folded edit distances alone: -0.0012, indistinguishable from zero on every source. The original distances already carry this; case folding fixes a problem (all-caps mentions) that is too rare in these corpora to move the average.
