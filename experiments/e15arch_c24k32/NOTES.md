...
no WikiDocs cap; this arm adds `--code-size 32`.
...
for S in 42 101 202 617 1848; do
  WANDB_MODE=offline uv run python tools/train.py train --epochs 15 --mix-dim 512 \
    ...
    --code-size 32 --seed $S \
    --metrics-out experiments/e15arch_c24k32/seed$S.json > experiments/e15arch_c24k32/seed$S.log 2>&1
done
```
...
code_size 8 -> 32. Nothing (+0.0013 ± 0.0063).
