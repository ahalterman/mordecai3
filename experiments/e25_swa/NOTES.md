...
no WikiDocs cap. This arm: SWA, average epochs 7-12 (`--epochs 12 --avg-params --avg-mode swa`).
...
for S in 42 101 202 617 1848; do
  WANDB_MODE=offline uv run python tools/train.py train --mix-dim 512 \
    ...
    --epochs 12 --avg-params --avg-mode swa --seed $S \
    --checkpoint-out experiments/e25_swa/seed$S.pt \
    --metrics-out experiments/e25_swa/seed$S.json > experiments/e25_swa/seed$S.log 2>&1
done
```
...
**The wave's training winner at 12 epochs.** Equal-weight average of the epoch-end weights from epoch 7 on, keeping the ordinary flat LR. +0.0055 EM* and +0.0020 acc@161* over e14_no_cf; +0.0042 EM* against the matched 12-epoch control, so this is the averaging, not the shorter schedule. It improves five of six sources (LGL +0.0108*, GWN +0.0123*, Synth +0.0067*, Prodigy +0.0015, WikiDocs +0.0030) and is flat on TR -- the closest thing in this campaign to a free, uniform gain, exactly as the error analysis predicted. Note the rewrite: the old --avg-params never evaluated or saved the averaged copy, pinned the LR at 0.05 via SWALR from epoch 6, and was the only caller of scheduler.step().
