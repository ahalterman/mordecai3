...
--label-smoothing 0.05`, no WikiDocs cap, plus `--enriched --feature-blocks "name,cue,sib,geo,shape,cf"`
(25 extra columns on gaz_info).
...
for S in 42 101 202 617 1848; do
  WANDB_MODE=offline uv run python tools/train.py train \
    ...
    --seed $S --enriched --feature-blocks "name,cue,sib,geo,shape,cf" \
    --metrics-out experiments/e14_no_prom/seed$S.json > experiments/e14_no_prom/seed$S.log 2>&1
done
```
...
All blocks except prom, keeping cf. +0.0292, about 0.005 below e14_no_cf: dropping population costs a little, dropping the case-folded distances costs nothing. GWN likes this arm best (+0.0183).
