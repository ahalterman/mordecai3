...
`--logits --mask-padding --oov-bucket-fix --modern-mlp `, epochs 15, WikiDocs cap none (all 15,065).
...
for S in 42 101 202 617 1848; do
  WANDB_MODE=offline uv run python tools/train.py train --epochs 15 --mix-dim 512 \
    ...
    --metrics-out experiments/e11_combo/seed$S.json > experiments/e11_combo/seed$S.log 2>&1
done
```
...
|---|---|
| Prodigy | -0.0406 ± 0.0108 * |
| TR | -0.0080 ± 0.0166 |
| LGL | +0.0058 ± 0.0047 * |
| GWN | -0.0198 ± 0.0080 * |
| Synth | +0.0126 ± 0.0073 * |
| WikiDocs | +0.0413 ± 0.0034 * |

## Verdict

Best of e7/e8/e9 (e9_short: 15 epochs) + best of e10 (no cap). EM back to parity (-0.0014, n.s.), acc@161 +0.0083*, at the same wall clock as baseline.
