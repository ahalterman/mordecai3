...
no WikiDocs cap. This arm: strip block + SWA (`--epochs 12 --avg-params + blocks ...,strip`).
...
for S in 42 101 202 617 1848; do
  WANDB_MODE=offline uv run python tools/train.py train --mix-dim 512 \
    ...
    --epochs 12 --avg-params + blocks ...,strip --seed $S \
    --checkpoint-out experiments/e27_strip_ep12/seed$S.pt \
    --metrics-out experiments/e27_strip_ep12/seed$S.json > experiments/e27_strip_ep12/seed$S.log 2>&1
done
```
...
Strip block + SWA, the configuration the brief specified. -0.0035 EM (n.s.) and -0.0026 acc@161* against its matched control e25_swa.
