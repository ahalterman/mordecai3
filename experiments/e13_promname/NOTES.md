...
--label-smoothing 0.05`, no WikiDocs cap, plus `--enriched --feature-blocks "prom,name"`
(7 extra columns on gaz_info).
...
for S in 42 101 202 617 1848; do
  WANDB_MODE=offline uv run python tools/train.py train \
    ...
    --seed $S --enriched --feature-blocks "prom,name" \
    --metrics-out experiments/e13_promname/seed$S.json > experiments/e13_promname/seed$S.log 2>&1
done
```
...
Population prominence plus exact-name matching: only +0.0035, and it is the one block pair that makes a source *worse* (GWN -0.0086*). Most of what population knows is apparently already implied by the geometry and sibling blocks. Synth (+0.0144*) is the source that likes it, which is precisely the cheat-sheet source we agreed not to judge on.
