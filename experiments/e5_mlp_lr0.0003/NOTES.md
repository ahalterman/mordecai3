...
Flags: `--lr 0.0003 --logits --mask-padding --oov-bucket-fix --modern-mlp`
...
WANDB_MODE=offline uv run python tools/train.py train --epochs 30 --mix-dim 512 \
  ...
  --seed $S --lr 0.0003 --logits --mask-padding --oov-bucket-fix --modern-mlp \
  --metrics-out experiments/e5_mlp_lr0.0003/seed$S.json > experiments/e5_mlp_lr0.0003/seed$S.log 2>&1
done
```
...
|---|---|
| Prodigy | -0.0499 ± 0.0159 * |
| TR | -0.0158 ± 0.0192 |
| LGL | -0.0134 ± 0.0112 * |
| GWN | -0.0336 ± 0.0120 * |
| Synth | -0.0049 ± 0.0082 |
| WikiDocs | -0.0181 ± 0.0076 * |
...
Test of the obvious hypothesis -- that removing the double softmax raised the
effective gradient scale, so lr=1e-3 is now too hot. Lowering the LR makes it
WORSE (-0.023), and moves the peak even earlier (epoch 10.6). The fixed model is
overfitting, not oscillating: the hypothesis is rejected.
