...
no WikiDocs cap. This arm: EMA of weights, decay 0.9 (`--epochs 12 --avg-params --avg-mode ema --ema-decay 0.9`).
...
for S in 42 101 202 617 1848; do
  WANDB_MODE=offline uv run python tools/train.py train --mix-dim 512 \
    ...
    --epochs 12 --avg-params --avg-mode ema --ema-decay 0.9 --seed $S \
    --checkpoint-out experiments/e25_ema/seed$S.pt \
    --metrics-out experiments/e25_ema/seed$S.json > experiments/e25_ema/seed$S.log 2>&1
done
```
...
Exponential moving average, decay 0.9/epoch. +0.0049 EM* -- statistically indistinguishable from SWA on exact match, but its acc@161 gain is not significant. Equal-weight averaging is the better-behaved of the two here, so SWA is the one to ship.
