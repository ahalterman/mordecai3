...
no WikiDocs cap; this arm adds `--listwise (4 heads)`.
...
for S in 42 101 202 617 1848; do
  WANDB_MODE=offline uv run python tools/train.py train --epochs 15 --mix-dim 512 \
    ...
    --listwise (4 heads) --seed $S \
    --metrics-out experiments/e16arch_listwise4/seed$S.json > experiments/e16arch_listwise4/seed$S.log 2>&1
done
```
...
Mask-aware self-attention over the 500 candidates, 4 heads. Much worse (-0.0356*), unstable across seeds (±0.034; one seed's loss ended at 2.5 vs 0.54 for the base) and 4-9x slower. Every source drops, WikiDocs -0.043*.
