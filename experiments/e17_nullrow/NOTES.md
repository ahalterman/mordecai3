...
no WikiDocs cap; this arm adds `--full-null-row`.
...
for S in 42 101 202 617 1848; do
  WANDB_MODE=offline uv run python tools/train.py train --epochs 15 --mix-dim 512 \
    ...
    --full-null-row --seed $S \
    --metrics-out experiments/e17_nullrow/seed$S.json > experiments/e17_nullrow/seed$S.log 2>&1
done
```
...
Audit item B2, and the answer is that the proposed fix is a **provable no-op**: every seed and every source is bit-identical to the base. Both layouts sentinel-overwrite row `max_choices-1`, so whether that row held the 500th real candidate or the gazetteer's NULL row makes no difference to any array the model sees -- the same candidates 0..498 compete either way. Measured on all 32,806 entities: 17.0% exceed the window (34.5% of TR, 41.0% of LGL, 31.2% of GWN -- not rare), but the gold is at the dropped index in **0** of them. B2 is a real layout wart that costs exactly zero accuracy at window 500. It would only matter at inference windows small enough for the gold to land in the last slot.
