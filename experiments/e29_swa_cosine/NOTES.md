...
no WikiDocs cap. This arm: SWA + the cosine schedule actually stepped (`--epochs 12 --avg-params --avg-mode swa --lr-schedule`).
...
for S in 42 101 202 617 1848; do
  WANDB_MODE=offline uv run python tools/train.py train --mix-dim 512 \
    ...
    --epochs 12 --avg-params --avg-mode swa --lr-schedule --seed $S \
    --checkpoint-out experiments/e29_swa_cosine/seed$S.pt \
    --metrics-out experiments/e29_swa_cosine/seed$S.json > experiments/e29_swa_cosine/seed$S.log 2>&1
done
```
...
SWA with the cosine schedule actually stepped. Slightly *worse* than SWA alone (+0.0047 vs +0.0055): decaying the LR reduces the spread of the points being averaged, which is the thing that makes averaging pay. Keep the flat LR.
