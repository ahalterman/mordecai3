...
no WikiDocs cap; this arm adds `--epochs 10`.
...
for S in 42 101 202 617 1848; do
  WANDB_MODE=offline uv run python tools/train.py train --epochs 15 --mix-dim 512 \
    ...
    --epochs 10 --seed $S \
    --metrics-out experiments/e23_ep10/seed$S.json > experiments/e23_ep10/seed$S.log 2>&1
done
```
...
10 epochs. Same small EM gain, no acc@161 gain, cheapest arm.
