...
`--logits --mask-padding --oov-bucket-fix --modern-mlp --label-smoothing 0.05`, epochs 15, WikiDocs cap none (all 15,065).
...
for S in 42 101 202 617 1848; do
  WANDB_MODE=offline uv run python tools/train.py train --epochs 15 --mix-dim 512 \
    ...
    --metrics-out experiments/e11b_combo_ls/seed$S.json > experiments/e11b_combo_ls/seed$S.log 2>&1
done
```
...
|---|---|
| Prodigy | -0.0394 ± 0.0120 * |
| TR | +0.0043 ± 0.0154 |
| LGL | +0.0148 ± 0.0061 * |
| GWN | -0.0038 ± 0.0104 |
| Synth | +0.0015 ± 0.0064 |
| WikiDocs | +0.0450 ± 0.0051 * |

## Verdict

**The recommended recipe.** Full fixed stack + label smoothing 0.05 + 15 epochs + uncapped WikiDocs. First config to beat baseline on both metrics: EM +0.0037 ± 0.0022*, acc@161 +0.0101 ± 0.0018*, in 53s (or 144s for all five seeds run concurrently). Label smoothing and the short schedule are complementary: smoothing lifts the peak, the short schedule stops at it, and the extra data is what makes 15 epochs enough.
