...
--label-smoothing 0.05`, no WikiDocs cap, plus `--enriched --feature-blocks "geo,shape"`
(13 extra columns on gaz_info).
...
for S in 42 101 202 617 1848; do
  WANDB_MODE=offline uv run python tools/train.py train \
    ...
    --seed $S --enriched --feature-blocks "geo,shape" \
    --metrics-out experiments/e13_geoshape/seed$S.json > experiments/e13_geoshape/seed$S.log 2>&1
done
```
...
Sibling-anchor geometry plus the set-shape gates: +0.0220 EM. LGL (+0.065) is where it lands, which is exactly the ambiguity-heavy source geometry should help. Synth is flat (+0.0001), a useful negative control -- the synthetic sets are a geometry/population cheat sheet, so a *real* geometry gain that shows up on LGL/TR and not on Synth is the honest kind. No sign of the 'geometry alone hurts name variants' problem here, because shape rides along as the prior said it must.
