...
`--logits --mask-padding --oov-bucket-fix --modern-mlp --label-smoothing 0.1`, epochs 15, WikiDocs cap none (all 15,065).
...
for S in 42 101 202 617 1848; do
  WANDB_MODE=offline uv run python tools/train.py train --epochs 15 --mix-dim 512 \
    ...
    --metrics-out experiments/e12_ls01_full15/seed$S.json > experiments/e12_ls01_full15/seed$S.log 2>&1
done
```
...
|---|---|
| Prodigy | -0.0439 ± 0.0095 * |
| TR | +0.0009 ± 0.0178 |
| LGL | +0.0144 ± 0.0065 * |
| GWN | -0.0025 ± 0.0095 |
| Synth | -0.0056 ± 0.0091 |
| WikiDocs | +0.0449 ± 0.0045 * |

## Verdict

Refinement around e11b: smoothing 0.1 instead of 0.05 is slightly worse (+0.0013 vs +0.0037). 0.05 is the right level.
