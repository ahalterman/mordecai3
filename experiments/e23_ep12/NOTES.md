...
no WikiDocs cap. This arm: 12 epochs (schedule control) (`--epochs 12`).
...
for S in 42 101 202 617 1848; do
  WANDB_MODE=offline uv run python tools/train.py train --mix-dim 512 \
    ...
    --epochs 12 --seed $S \
    --checkpoint-out experiments/e23_ep12/seed$S.pt \
    --metrics-out experiments/e23_ep12/seed$S.json > experiments/e23_ep12/seed$S.log 2>&1
done
```
...
Schedule control for the wave: the e14 recipe at 12 epochs, carried over from wave 3. Everything below that changes the epoch count is compared to this as well as to e14_no_cf.
