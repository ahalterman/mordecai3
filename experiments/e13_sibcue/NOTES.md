...
--label-smoothing 0.05`, no WikiDocs cap, plus `--enriched --feature-blocks "sib,cue"`
(6 extra columns on gaz_info).
...
for S in 42 101 202 617 1848; do
  WANDB_MODE=offline uv run python tools/train.py train \
    ...
    --seed $S --enriched --feature-blocks "sib,cue" \
    --metrics-out experiments/e13_sibcue/seed$S.json > experiments/e13_sibcue/seed$S.log 2>&1
done
```
...
Sibling parent-name matching plus the admin cue: +0.0223, essentially tied with geo+shape for the biggest single contribution, and the cheapest (6 features). TR +0.0447, LGL +0.0479. 'Springfield is the Illinois one when Illinois is in the article' is doing real work.
