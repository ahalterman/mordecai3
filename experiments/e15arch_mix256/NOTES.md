...
no WikiDocs cap; this arm adds `--mix-dim 256`.
...
for S in 42 101 202 617 1848; do
  WANDB_MODE=offline uv run python tools/train.py train --epochs 15 --mix-dim 512 \
    ...
    --mix-dim 256 --seed $S \
    --metrics-out experiments/e15arch_mix256/seed$S.json > experiments/e15arch_mix256/seed$S.log 2>&1
done
```
...
Halving the mix width is the only capacity change that is significantly worse on acc@161 (-0.0013*), so 512 is at or just past the knee -- but going up does not help either.
