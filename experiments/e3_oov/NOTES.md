...
Flags: `--logits --mask-padding --oov-bucket-fix`
...
WANDB_MODE=offline uv run python tools/train.py train --epochs 30 --mix-dim 512 \
  ...
  --seed $S --logits --mask-padding --oov-bucket-fix \
  --metrics-out experiments/e3_oov/seed$S.json > experiments/e3_oov/seed$S.log 2>&1
done
```
...
|---|---|
| Prodigy | -0.0090 ± 0.0152 |
| TR | +0.0006 ± 0.0175 |
| LGL | -0.0152 ± 0.0047 * |
| GWN | -0.0180 ± 0.0100 * |
| Synth | -0.0072 ± 0.0082 |
| WikiDocs | -0.0167 ± 0.0039 * |
...
Splitting the OOV feature code off the NULL bucket (index 51 rather than 52) and
giving the reserved last row the NULL country recovers ~0.003 exact match and
~0.004 acc@161 versus e2 -- the right sign, but each individually inside noise.
acc@161 is no longer significantly below baseline.
