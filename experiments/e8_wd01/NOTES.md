...
`--logits --mask-padding --oov-bucket-fix --modern-mlp --weight-decay 0.1`, epochs 30, WikiDocs cap 2000.
...
for S in 42 101 202 617 1848; do
  WANDB_MODE=offline uv run python tools/train.py train --epochs 30 --mix-dim 512 \
    ...
    --seed $S --logits --mask-padding --oov-bucket-fix --modern-mlp --weight-decay 0.1 \
    --metrics-out experiments/e8_wd01/seed$S.json > experiments/e8_wd01/seed$S.log 2>&1
done
```
...
|---|---|
| Prodigy | -0.0222 ± 0.0123 * |
| TR | -0.0009 ± 0.0104 |
| LGL | -0.0041 ± 0.0093 |
| GWN | -0.0161 ± 0.0137 * |
| Synth | -0.0013 ± 0.0065 |
| WikiDocs | -0.0095 ± 0.0103 |

## Verdict

Also nothing, with more seed variance. Weight decay is not the missing regularizer.
