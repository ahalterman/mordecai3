...
`--logits --mask-padding --oov-bucket-fix --modern-mlp `, epochs 15, WikiDocs cap 2000.
...
for S in 42 101 202 617 1848; do
  WANDB_MODE=offline uv run python tools/train.py train --epochs 15 --mix-dim 512 \
    ...
    --metrics-out experiments/e9_short/seed$S.json > experiments/e9_short/seed$S.log 2>&1
done
```
...
|---|---|
| Prodigy | -0.0216 ± 0.0110 * |
| TR | +0.0061 ± 0.0102 |
| LGL | -0.0049 ± 0.0025 * |
| GWN | -0.0074 ± 0.0113 |
| Synth | +0.0058 ± 0.0054 * |
| WikiDocs | -0.0058 ± 0.0054 * |

## Verdict

'Just stop earlier' works better than either principled regularizer: -0.0046 EM and acc@161 back to parity (+0.0028), at 43s vs 55s. The fixed stack's peak region is epochs 11-16, so scoring epochs 11-15 captures most of it.
