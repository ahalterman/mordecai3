...
`--logits --mask-padding --oov-bucket-fix --modern-mlp --label-smoothing 0.05`, epochs 22, WikiDocs cap none (all 15,065).
...
for S in 42 101 202 617 1848; do
  WANDB_MODE=offline uv run python tools/train.py train --epochs 22 --mix-dim 512 \
    --dataset-names "Prodigy, TR, LGL, GWN, Synth, WikiDocs"  \
    --seed $S --logits --mask-padding --oov-bucket-fix --modern-mlp --label-smoothing 0.05 \
    --metrics-out experiments/e12_ls005_full22/seed$S.json > experiments/e12_ls005_full22/seed$S.log 2>&1
done
```
...
|---|---|
| Prodigy | -0.0413 ± 0.0092 * |
| TR | +0.0049 ± 0.0148 |
| LGL | +0.0084 ± 0.0096 |
| GWN | -0.0075 ± 0.0091 |
| Synth | -0.0039 ± 0.0083 |
| WikiDocs | +0.0454 ± 0.0034 * |

## Verdict

Refinement around e11b: 22 epochs instead of 15 is slightly worse (+0.0010 vs +0.0037) though it has the highest peak (0.8910). 15 epochs is the right length.
