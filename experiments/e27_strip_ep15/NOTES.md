...
no WikiDocs cap. This arm: strip block + SWA at 15 epochs (`--epochs 15 --avg-params + blocks ...,strip`).
...
for S in 42 101 202 617 1848; do
  WANDB_MODE=offline uv run python tools/train.py train --mix-dim 512 \
    ...
    --epochs 15 --avg-params + blocks ...,strip --seed $S \
    --checkpoint-out experiments/e27_strip_ep15/seed$S.pt \
    --metrics-out experiments/e27_strip_ep15/seed$S.json > experiments/e27_strip_ep15/seed$S.log 2>&1
done
```
...
Strip block + SWA at 15 epochs. -0.0030 EM (n.s.) and -0.0031 acc@161* against e29_swa_ep15, and **worse on twin credit too** (93.59% vs 93.79% at K=1), which is the metric these features were designed to move. The stripped-name A/P signal is apparently already implied by what `prom`, `name` and `cue` carry; adding it as three more raw columns costs more than it pays. Verified not to be a data artifact: the 30 base features are byte-identical across enrichment rounds 4/5/6 (a control rerun of e25_swa seed42 on the rebuilt cache reproduces its metrics exactly).
