...
no WikiDocs cap; this arm adds `--mix-dim 1024`.
...
for S in 42 101 202 617 1848; do
  WANDB_MODE=offline uv run python tools/train.py train --epochs 15 --mix-dim 512 \
    ...
    --mix-dim 1024 --seed $S \
    --metrics-out experiments/e15arch_mix1024/seed$S.json > experiments/e15arch_mix1024/seed$S.log 2>&1
done
```
...
Doubling the mix width: nothing, and Prodigy significantly worse (-0.0143*).
