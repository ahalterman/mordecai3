...
`--logits --mask-padding --oov-bucket-fix --modern-mlp --label-smoothing 0.1`, epochs 30, WikiDocs cap 2000.
...
for S in 42 101 202 617 1848; do
  WANDB_MODE=offline uv run python tools/train.py train --epochs 30 --mix-dim 512 \
    ...
    --seed $S --logits --mask-padding --oov-bucket-fix --modern-mlp --label-smoothing 0.1 \
    --metrics-out experiments/e7_ls01/seed$S.json > experiments/e7_ls01/seed$S.log 2>&1
done
```
...
|---|---|
| Prodigy | -0.0533 ± 0.0154 * |
| TR | +0.0164 ± 0.0138 * |
| LGL | +0.0008 ± 0.0114 |
| GWN | -0.0119 ± 0.0052 * |
| Synth | -0.0051 ± 0.0107 |
| WikiDocs | +0.0033 ± 0.0019 * |

## Verdict

Highest peak of any 30-epoch config (0.8912, epoch 11) but noisier and no better at the endpoint. Prodigy takes the worst hit of the smoothing arms (-0.053).
