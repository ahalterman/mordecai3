...
`--logits --mask-padding --oov-bucket-fix --modern-mlp --label-smoothing 0.2`, epochs 30, WikiDocs cap 2000.
...
for S in 42 101 202 617 1848; do
  WANDB_MODE=offline uv run python tools/train.py train --epochs 30 --mix-dim 512 \
    ...
    --seed $S --logits --mask-padding --oov-bucket-fix --modern-mlp --label-smoothing 0.2 \
    --metrics-out experiments/e7_ls02/seed$S.json > experiments/e7_ls02/seed$S.log 2>&1
done
```
...
|---|---|
| Prodigy | -0.0526 ± 0.0158 * |
| TR | +0.0201 ± 0.0125 * |
| LGL | -0.0001 ± 0.0086 |
| GWN | -0.0101 ± 0.0046 * |
| Synth | -0.0128 ± 0.0105 * |
| WikiDocs | +0.0024 ± 0.0033 |

## Verdict

Too much: peak arrives at epoch 8.8 and acc@161 is significantly below baseline. 0.05-0.1 is the useful range.
