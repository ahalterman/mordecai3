...
Flags: `--lr 0.0001 --logits --mask-padding --oov-bucket-fix --modern-mlp`
...
WANDB_MODE=offline uv run python tools/train.py train --epochs 30 --mix-dim 512 \
  ...
  --seed $S --lr 0.0001 --logits --mask-padding --oov-bucket-fix --modern-mlp \
  --metrics-out experiments/e5_mlp_lr0.0001/seed$S.json > experiments/e5_mlp_lr0.0001/seed$S.log 2>&1
done
```
...
|---|---|
| Prodigy | -0.0482 ± 0.0156 * |
| TR | -0.0156 ± 0.0162 |
| LGL | -0.0234 ± 0.0069 * |
| GWN | -0.0380 ± 0.0082 * |
| Synth | -0.0153 ± 0.0092 * |
| WikiDocs | -0.0278 ± 0.0045 * |
...
Same test, lr=1e-4. Worse again (-0.028). Confirms the LR explanation is wrong.
