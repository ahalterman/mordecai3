...
no WikiDocs cap. This arm: restore train() after each eval (dropout bug) (`--epochs 12 --train-after-eval`).
...
for S in 42 101 202 617 1848; do
  WANDB_MODE=offline uv run python tools/train.py train --mix-dim 512 \
    ...
    --epochs 12 --train-after-eval --seed $S \
    --checkpoint-out experiments/e28_trainmode/seed$S.pt \
    --metrics-out experiments/e28_trainmode/seed$S.json > experiments/e28_trainmode/seed$S.log 2>&1
done
```
...
A latent bug I found while rewriting the loop: `error_utils.evaluate_results` calls `model.eval()` and nothing ever calls `model.train()` again, so **dropout has been inert from epoch 2 onward in every run of this campaign**. Restoring train mode costs -0.0008 (n.s.) overall and significantly hurts Prodigy (-0.0101*) and TR (-0.0121*). So the bug is harmless-to-helpful and should be left alone -- but it explains wave 1's e6 result, where raising dropout to 0.5 did nothing: dropout was never running.
