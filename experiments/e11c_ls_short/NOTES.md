...
`--logits --mask-padding --oov-bucket-fix --modern-mlp --label-smoothing 0.05`, epochs 15, WikiDocs cap 2000.
...
for S in 42 101 202 617 1848; do
  WANDB_MODE=offline uv run python tools/train.py train --epochs 15 --mix-dim 512 \
    ...
    --metrics-out experiments/e11c_ls_short/seed$S.json > experiments/e11c_ls_short/seed$S.log 2>&1
done
```
...
|---|---|
| Prodigy | -0.0284 ± 0.0140 * |
| TR | +0.0249 ± 0.0110 * |
| LGL | +0.0043 ± 0.0064 |
| GWN | -0.0016 ± 0.0067 |
| Synth | +0.0039 ± 0.0062 |
| WikiDocs | +0.0033 ± 0.0038 |

## Verdict

Same recipe with WikiDocs still capped at 2000, to separate the data effect: EM +0.0011 (n.s.), acc@161 +0.0052*. So roughly two-thirds of e11b's gain comes from the uncapped data and one-third from smoothing+schedule. Cheapest config in the campaign at 34s/run.
