...
no WikiDocs cap. This arm: strip block, no averaging (`--epochs 12 + blocks ...,strip`).
...
for S in 42 101 202 617 1848; do
  WANDB_MODE=offline uv run python tools/train.py train --mix-dim 512 \
    ...
    --epochs 12 + blocks ...,strip --seed $S \
    --checkpoint-out experiments/e27_strip_nosw/seed$S.pt \
    --metrics-out experiments/e27_strip_nosw/seed$S.json > experiments/e27_strip_nosw/seed$S.log 2>&1
done
```
...
Enrichment round 5+6 'strip' block (ap_twin_stripped, ap_twin_stripped_wide, is_historical) without weight averaging. -0.0021 EM against the matched control; TR -0.0123*.
