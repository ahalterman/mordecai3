...
no WikiDocs cap; this arm adds `--country-size 128`.
...
for S in 42 101 202 617 1848; do
  WANDB_MODE=offline uv run python tools/train.py train --epochs 15 --mix-dim 512 \
    ...
    --country-size 128 --seed $S \
    --metrics-out experiments/e15arch_c128k8/seed$S.json > experiments/e15arch_c128k8/seed$S.log 2>&1
done
```
...
country_size 128. Nothing; Prodigy drifts down (-0.0174, n.s.).
