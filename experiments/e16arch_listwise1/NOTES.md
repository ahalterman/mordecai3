...
no WikiDocs cap; this arm adds `--listwise --listwise-heads 1`.
...
for S in 42 101 202 617 1848; do
  WANDB_MODE=offline uv run python tools/train.py train --epochs 15 --mix-dim 512 \
    ...
    --listwise --listwise-heads 1 --seed $S \
    --metrics-out experiments/e16arch_listwise1/seed$S.json > experiments/e16arch_listwise1/seed$S.log 2>&1
done
```
...
Single-head version. Worse still (-0.0576) with enormous seed spread. Letting candidates attend to each other before scoring destabilises training rather than making them compete.
