...
no WikiDocs cap; this arm adds `--epochs 12 --aux-country-weight 0.2`.
...
for S in 42 101 202 617 1848; do
  WANDB_MODE=offline uv run python tools/train.py train --epochs 15 --mix-dim 512 \
    ...
    --epochs 12 --aux-country-weight 0.2 --seed $S \
    --metrics-out experiments/e23_ep12_country02/seed$S.json > experiments/e23_ep12_country02/seed$S.log 2>&1
done
```
...
12 epochs plus the country auxiliary head at 0.2 -- the two marginal winners stacked. Best acc@161 in the wave (+0.0024*) and best absolute (0.9655), EM +0.0015 (n.s.), LGL +0.0145*. But Prodigy drops -0.0195, so this trades the weakest source for the ambiguous ones.
