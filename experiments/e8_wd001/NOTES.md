...
`--logits --mask-padding --oov-bucket-fix --modern-mlp --weight-decay 0.01`, epochs 30, WikiDocs cap 2000.
...
for S in 42 101 202 617 1848; do
  WANDB_MODE=offline uv run python tools/train.py train --epochs 30 --mix-dim 512 \
    ...
    --seed $S --logits --mask-padding --oov-bucket-fix --modern-mlp --weight-decay 0.01 \
    --metrics-out experiments/e8_wd001/seed$S.json > experiments/e8_wd001/seed$S.log 2>&1
done
```
...
|---|---|
| Prodigy | -0.0206 ± 0.0094 * |
| TR | -0.0009 ± 0.0104 |
| LGL | -0.0079 ± 0.0066 * |
| GWN | -0.0155 ± 0.0152 * |
| Synth | -0.0007 ± 0.0058 |
| WikiDocs | -0.0093 ± 0.0070 * |

## Verdict

No effect (-0.0091 vs e4's -0.0093). AdamW decay does not reproduce what the double softmax was doing.
